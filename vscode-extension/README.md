...
- **Pre-merge conflict check** — before merging, Grove merges the selected branches in memory with `git merge-tree`, one after another in the order it will merge them, and lists the files git reports as conflicting, naming the branch (or the base branch) each one conflicts with. Two agents that changed the same lines are caught even when the base branch has not moved. Nothing in your checkout, index or branches changes. The report also lists files changed on both the base branch and a worktree branch since they diverged

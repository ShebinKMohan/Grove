...
- **You can resolve conflicts in the editor** — the conflict prompt was a modal dialog, which blocks the editor. It is now a notification that stays open while you edit. Closing it stops the sequence and leaves the merge in progress; only "Skip This Branch" and "Abort All" run `git merge --abort`, and if that fails the sequence stops instead of merging the next branch on top. Only one merge sequence runs at a time
